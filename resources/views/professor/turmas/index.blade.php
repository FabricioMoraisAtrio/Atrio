...
<div style="margin-bottom: 24px;">
    <h1 style="font-size: 22px; font-weight: 700; color: var(--text-1); margin: 0 0 4px;">Turmas</h1>
    <p style="font-size: 13px; color: var(--text-4); margin: 0;">Turmas associadas à sua escola</p>
</div>
...
@if($turmas->isEmpty())
    <div style="background: var(--bg-card); border: 1px solid var(--border-sub); border-radius: 12px; padding: 48px; text-align: center;">
        ...
        <p style="font-size: 14px; color: var(--text-4); margin: 0;">Nenhuma turma cadastrada.</p>
    </div>
@else
    <div style="display: flex; flex-direction: column; gap: 10px;">
        ...
        @endphp
        <div style="background: var(--bg-card); border: 1px solid var(--border-sub); border-radius: 14px; overflow: hidden;">
            ...
            <div onclick="toggleTurma({{ $turma->id }})"
                ...
                <div style="display: flex; align-items: flex-start; gap: 14px; flex: 1;">
                    ...
                    <div>
                        <div style="font-size: 17px; font-weight: 700; color: var(--text-1); margin-bottom: 4px;">{{ $turma->name }}</div>
                        <div style="display: flex; gap: 8px; align-items: center;">
                            ...
                            <span style="font-size: 12px; color: var(--text-4);">Ano letivo {{ $turma->year }}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                        <thead>
                            <tr style="background: var(--bg-subtle);">
                                ...
                                <th style="text-align: center; padding: 10px 10px; font-size: 10px; font-weight: 600; color: var(--text-4); text-transform: uppercase; letter-spacing: 0.5px;">PEI</th>
                                <th style="text-align: center; padding: 10px 10px; font-size: 10px; font-weight: 600; color: var(--text-4); text-transform: uppercase; letter-spacing: 0.5px;">Laudo</th>
                        ...
                        <tbody>
                            ...
                            <tr style="border-top: 1px solid var(--border-sub);"
                                ...
                                <td style="padding: 12px 16px;">
                                    @if($aluno->is_atypical)
                                        @if($aluno->is_publico_alvo)
                                            <span style="background: var(--purple-bg); color: var(--purple); font-size: 10px; font-weight: 600; padding: 2px 7px; border-radius: 20px;">Público Alvo</span>
                                        @else
                                        ...
                                        @endif
                                    @else
                                        <span style="background: var(--bg-subtle); color: var(--text-3); font-size: 10px; padding: 2px 7px; border-radius: 20px;">Típico</span>
                                    @endif
                                </td>
                                @foreach([
                                    ['ok' => $temEC],
                                ...
                                <td style="padding: 12px 10px; text-align: center;">
                                    @if($col['ok'])
                                        <span style="background: var(--success-bg); color: var(--success); font-size: 11px; font-weight: 600; padding: 2px 7px; border-radius: 20px;">✓</span>
                                    @else
                                        <span style="background: var(--bg-subtle); color: var(--text-4); font-size: 11px; padding: 2px 7px; border-radius: 20px;">—</span>
                                    @endif
                                </td>
                                @endforeach
