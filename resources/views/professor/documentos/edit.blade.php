...
<div style="max-width: 1000px;">
    <div style="margin-bottom: 24px;">
        ...
        <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <h1 style="font-size: 22px; font-weight: 700; color: var(--text-1); margin: 0;">{{ $documento->student->name }}</h1>
        </div>
    </div>

    <div style="background: var(--bg-card); border-radius: 12px; border: 1px solid var(--border-sub); padding: 28px;">
        <form method="POST" action="{{ route('professor.documentos.update', $documento) }}">
            @csrf @method('PUT')
