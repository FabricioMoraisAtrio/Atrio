...
<div style="margin-bottom:24px;">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;">
        <div>
            <h1 style="font-size:22px;font-weight:700;color:var(--text-1);margin:0 0 4px;">{{ $subject->name }}</h1>
        ...
        <a href="{{ route('secretaria.subjects.edit', $subject) }}"
           style="font-size:13px;color:var(--text-3);text-decoration:none;border:1px solid var(--border);padding:8px 16px;border-radius:8px;">
            Editar matéria
        </a>
    </div>
</div>
...
{{-- Metas do inventário --}}
<div style="background:var(--bg-card);border-radius:12px;border:1px solid var(--border-sub);padding:24px;">
    <div style="margin-bottom:20px;">
        <h3 style="font-size:15px;font-weight:700;color:var(--text-1);margin:0 0 4px;">Metas do Inventário</h3>
    ...
    <form method="POST" action="{{ route('secretaria.subjects.saveItems', $subject) }}" id="formMetas">
        ...
        {{-- Cabeçalho da tabela --}}
        <div style="display:grid;grid-template-columns:28px 1fr 220px 32px;gap:8px;padding:0 4px 8px;border-bottom:1px solid var(--border-sub);margin-bottom:8px;">
            <div></div>
            <div style="font-size:11px;font-weight:600;color:var(--text-4);text-transform:uppercase;letter-spacing:0.5px;">Meta / Objetivo</div>
            <div style="font-size:11px;font-weight:600;color:var(--text-4);text-transform:uppercase;letter-spacing:0.5px;">Categoria</div>
            <div></div>
        </div>
        ...
        <div id="metas-list" style="display:flex;flex-direction:column;gap:6px;margin-bottom:20px;">
            @foreach($subject->inventoryItems as $item)
                <div class="meta-row" style="display:grid;grid-template-columns:28px 1fr 220px 32px;gap:8px;align-items:center;">
                    <div style="cursor:grab;color:var(--text-4);flex-shrink:0;" title="Arrastar">
                    ...
                    <input type="text" name="metas[{{ $loop->index }}][texto]" value="{{ $item->meta }}" required
                           ...
                           style="width:100%;border:1px solid var(--border);border-radius:8px;padding:9px 12px;font-size:13px;color:var(--text-2);outline:none;box-sizing:border-box;"
                           onfocus="this.style.borderColor='var(--accent)'" onblur="this.style.borderColor='var(--border)'">
                    <select name="metas[{{ $loop->index }}][categoria]"
                            style="width:100%;border:1px solid var(--border);border-radius:8px;padding:9px 10px;font-size:12px;color:var(--text-2);outline:none;background:var(--bg-card);cursor:pointer;"
        ...
        <button type="button" onclick="adicionarMeta()"
                style="background:var(--bg-subtle);color:var(--text-2);border:1px dashed var(--border);padding:9px 16px;border-radius:8px;font-size:13px;font-weight:500;cursor:pointer;margin-bottom:20px;display:flex;align-items:center;gap:6px;">
