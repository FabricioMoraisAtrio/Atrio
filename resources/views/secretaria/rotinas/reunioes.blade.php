...
<div style="max-width: 900px;">
    ...
    <div style="margin-bottom: 20px;">
        <h1 style="font-size: 22px; font-weight: 700; color: var(--text-1); margin: 0 0 4px;">Reuniões / Atas</h1>
        <p style="font-size: 13px; color: var(--text-3); margin: 0;">
            Registre e acompanhe as reuniões (PEI, família, equipe, devolutivas) de cada {{ strtolower(term('aluno')) }}.
            <strong>{{ $totalReunioes }}</strong> reunião(ões) registrada(s).
        </p>
    </div>
            ...
            <a href="{{ route('secretaria.alunos.reunioes.index', $aluno) }}"
               ...
               onmouseover="this.style.background='var(--bg-subtle)'" onmouseout="this.style.background='transparent'">
                <div style="display: flex; align-items: center; gap: 12px; min-width: 0;">
                    ...
                    <div style="min-width: 0;">
                        ...
                        <p style="font-size: 12px; color: var(--text-4); margin: 0;">
                            @if($aluno->meetings_count > 0)
                                {{ $aluno->meetings_count }} reunião(ões) · última em {{ \Illuminate\Support\Carbon::parse($aluno->meetings_max_data)->format('d/m/Y') }}
                            @else
                ...
                <div style="display: flex; align-items: center; gap: 10px; flex-shrink: 0;">
                    @if($aluno->meetings_count > 0)
                        <span style="background: var(--accent-bg); color: var(--accent-text); font-size: 12px; font-weight: 700; padding: 3px 10px; border-radius: 20px;">{{ $aluno->meetings_count }}</span>
                    @endif
