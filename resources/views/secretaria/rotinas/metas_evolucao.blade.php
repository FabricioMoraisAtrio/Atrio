@extends('layouts.app')
@section('title', 'Evolução de Metas')
...
<div style="max-width: 900px;">
    ...
    <div style="margin-bottom: 20px;">
        <h1 style="font-size: 22px; font-weight: 700; color: var(--text-1); margin: 0 0 4px;">Evolução de Metas — {{ $ano }}</h1>
        <p style="font-size: 13px; color: var(--text-3); margin: 0;">
            Acompanhe a evolução bimestral das metas do PEI de cada {{ strtolower(term('aluno')) }}.
        </p>
    </div>
    ...
    <div style="background: var(--bg-card); border: 1px solid var(--border-sub); border-radius: 12px; overflow: hidden;">
        @forelse($alunos as $aluno)
            <a href="{{ route('secretaria.alunos.metas-evolucao.edit', $aluno) }}"
               ...
               onmouseover="this.style.background='var(--bg-subtle)'" onmouseout="this.style.background='transparent'">
                <div style="display: flex; align-items: center; gap: 12px; min-width: 0;">
                    ...
                    <div style="min-width: 0;">
                        ...
                        <p style="font-size: 12px; color: var(--text-4); margin: 0;">
                            @if($aluno->academic_goals_count > 0)
                                {{ $aluno->academic_goals_count }} meta(s) no PEI
                            @else
                ...
                <div style="display: flex; align-items: center; gap: 10px; flex-shrink: 0;">
                    @if($aluno->academic_goals_count > 0)
                        <span style="background: var(--accent-bg); color: var(--accent-text); font-size: 12px; font-weight: 700; padding: 3px 10px; border-radius: 20px;">{{ $aluno->academic_goals_count }}</span>
                    @else
                        <span style="background: var(--bg-subtle); color: var(--text-4); font-size: 11px; font-weight: 600; padding: 3px 10px; border-radius: 20px;">sem metas</span>
                    @endif
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--text-4)" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg>
                </div>
            </a>
        @empty
