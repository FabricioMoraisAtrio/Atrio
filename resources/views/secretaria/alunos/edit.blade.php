...
<div style="max-width: 560px;">
    ...
    <div style="background: #fff; border-radius: 12px; border: 1px solid #F3F4F6; padding: 28px;">
        <form method="POST" action="{{ route('secretaria.alunos.update', $aluno) }}" enctype="multipart/form-data">
            ...
            {{-- Responsável 1 --}}
            <div style="border: 1px solid #F3F4F6; border-radius: 10px; padding: 16px 20px; margin-bottom: 16px;">
                <p style="font-size: 11px; font-weight: 600; color: #6B7280; letter-spacing: 1px; text-transform: uppercase; margin: 0 0 14px;">Responsável</p>
                <div style="margin-bottom: 16px;">
                    <label style="display: block; font-size: 11px; font-weight: 600; color: #6B7280; margin-bottom: 6px;">Nome completo</label>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                    <div>
                        <label style="display: block; font-size: 11px; font-weight: 600; color: #6B7280; margin-bottom: 6px;">E-mail</label>
                        <input type="email" name="responsavel_email" value="{{ old('responsavel_email', $aluno->responsavel_email) }}"
                               placeholder="email@exemplo.com"
                               style="width: 100%; border: none; border-bottom: 2px solid #E5E7EB; padding: 8px 0; font-size: 14px; color: #111827; outline: none; background: transparent; box-sizing: border-box;"
                               onfocus="this.style.borderColor='#004B8D'" onblur="this.style.borderColor='#E5E7EB'">
                        @error('responsavel_email')<p style="font-size:11px;color:#EF4444;margin-top:3px;">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label style="display: block; font-size: 11px; font-weight: 600; color: #6B7280; margin-bottom: 6px;">Telefone / WhatsApp</label>
            ...
            {{-- Responsável 2 --}}
            <div style="border: 1px solid #F3F4F6; border-radius: 10px; padding: 16px 20px; margin-bottom: 20px;">
                <p style="font-size: 11px; font-weight: 600; color: #6B7280; letter-spacing: 1px; text-transform: uppercase; margin: 0 0 14px;">2º Responsável <span style="font-weight:400;color:#9CA3AF;">(opcional)</span></p>
                <div style="margin-bottom: 16px;">
                    <label style="display: block; font-size: 11px; font-weight: 600; color: #6B7280; margin-bottom: 6px;">Nome completo</label>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                    <div>
                        ...
                        <input type="email" name="responsavel_2_email" value="{{ old('responsavel_2_email', $aluno->responsavel_2_email) }}"
                               ...
                               style="width: 100%; border: none; border-bottom: 2px solid #E5E7EB; padding: 8px 0; font-size: 14px; color: #111827; outline: none; background: transparent; box-sizing: border-box;"
                               onfocus="this.style.borderColor='#004B8D'" onblur="this.style.borderColor='#E5E7EB'">
                        @error('responsavel_2_email')<p style="font-size:11px;color:#EF4444;margin-top:3px;">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label style="display: block; font-size: 11px; font-weight: 600; color: #6B7280; margin-bottom: 6px;">Telefone / WhatsApp</label>
            ...
            <div style="border: 1px solid #F3F4F6; border-radius: 10px; padding: 20px; margin-bottom: 20px;">
                <p style="font-size: 11px; font-weight: 600; color: #6B7280; letter-spacing: 1px; text-transform: uppercase; margin: 0 0 12px;">Turma</p>
                @if($aluno->schoolClasses->isNotEmpty())
                <div style="display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 12px;">
                    @foreach($aluno->schoolClasses as $turmaVinculada)
                    <span style="background: #E8F0F9; color: #004B8D; font-size: 12px; font-weight: 600; padding: 4px 12px; border-radius: 20px; display: inline-flex; align-items: center; gap: 6px;">
                ...
                <div style="display: flex; gap: 10px; align-items: center;">
                    <select name="school_class_id"
                            style="flex: 1; border: 1px solid #E5E7EB; border-radius: 8px; padding: 8px 12px; font-size: 13px; color: #374151; outline: none; background: #fff;">
                ...
                <p style="font-size: 11px; color: #9CA3AF; margin: 6px 0 0;">Selecionar uma nova turma substitui a atual.</p>
            </div>
            ...
            <div style="border: 1px solid #F3F4F6; border-radius: 10px; padding: 20px;">
                ...
                <div id="atypical_fields" style="{{ old('is_atypical', $aluno->is_atypical) ? '' : 'display:none;' }} margin-top: 20px;">
                    ...
                    {{-- Grupo: Público Alvo --}}
                    <div style="border-radius: 10px; border: 1.5px solid #D8B4FE; overflow: hidden; margin-bottom: 14px;">
                        <div style="background: #F5F3FF; padding: 8px 12px; border-bottom: 1px solid #D8B4FE;">
                            <span style="font-size: 10px; font-weight: 700; color: #6D28D9; letter-spacing: 0.8px; text-transform: uppercase;">Público Alvo — habilitam PEI</span>
                        </div>
                        <div style="padding: 10px; display: grid; grid-template-columns: 1fr 1fr; gap: 6px;">
                            @foreach($transtornosPA as $field => [$sigla, $nome])
                                <label style="display: flex; align-items: flex-start; gap: 8px; cursor: pointer; padding: 8px 10px; border-radius: 8px; border: 1px solid #EDE9FE; background: #FAFAFF; transition: border-color 0.15s;"
                                       onmouseover="this.style.borderColor='#7C3AED'; this.style.background='#F5F3FF'"
                                       onmouseout="this.style.borderColor='#EDE9FE'; this.style.background='#FAFAFF'">
                                    ...
                                    <div>
                                        <span style="font-size: 12px; font-weight: 700; color: #5B21B6; display: block;">{{ $sigla }}</span>
                                        <span style="font-size: 11px; color: #6B7280; line-height: 1.3;">{{ $nome }}</span>
                                    </div>
                                </label>
                            @endforeach
                        </div>
                    ...
                    <div style="border-radius: 10px; border: 1px solid #E5E7EB; overflow: hidden; margin-bottom: 16px;">
                        <div style="background: #F9FAFB; padding: 8px 12px; border-bottom: 1px solid #E5E7EB;">
                            <span style="font-size: 10px; font-weight: 700; color: #6B7280; letter-spacing: 0.8px; text-transform: uppercase;">Outras condições atípicas</span>
                        </div>
                        <div style="padding: 10px; display: grid; grid-template-columns: 1fr 1fr; gap: 6px;">
                            @foreach($transtornosOutros as $field => [$sigla, $nome])
                                <label style="display: flex; align-items: flex-start; gap: 8px; cursor: pointer; padding: 8px 10px; border-radius: 8px; border: 1px solid #F3F4F6; background: #fff;"
                                       onmouseover="this.style.borderColor='#004B8D'; this.style.background='#F8FAFF'"
                                    ...
                                    <div>
                                        <span style="font-size: 12px; font-weight: 700; color: #004B8D; display: block;">{{ $sigla }}</span>
                                        <span style="font-size: 11px; color: #6B7280; line-height: 1.3;">{{ $nome }}</span>
                                    </div>
                                </label>
                            @endforeach
