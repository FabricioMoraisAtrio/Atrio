...
<div style="max-width: 480px;">
    <div style="margin-bottom: 24px;">
        ...
        <h1 style="font-size: 22px; font-weight: 700; color: var(--text-1); margin: 0;">Nova turma</h1>
    </div>

    <div style="background: var(--bg-card); border-radius: 12px; border: 1px solid var(--border-sub); padding: 28px;">
        <form method="POST" action="{{ route('secretaria.turmas.store') }}">
            @csrf
            ...
            <div style="margin-bottom: 20px;">
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder="Ex: 7º A"
                       style="width: 100%; border: none; border-bottom: 2px solid {{ $errors->has('name') ? '#EF4444' : 'var(--border)' }}; padding: 8px 0; font-size: 14px; color: var(--text-1); outline: none; background: transparent; box-sizing: border-box;"
            ...
            <div style="margin-bottom: 20px;">
                ...
                <select name="shift"
                    ...
                    @foreach(['Matutino','Vespertino','Noturno'] as $turno)
                        <option value="{{ $turno }}" {{ old('shift') == $turno ? 'selected' : '' }}>{{ $turno }}</option>
                    @endforeach
                </select>
                @error('shift')<p style="font-size: 12px; color: var(--danger); margin-top: 4px;">{{ $message }}</p>@enderror
            </div>
            ...
            <div style="margin-bottom: 28px;">
                ...
                <input type="number" name="year" value="{{ old('year', date('Y')) }}" min="2020" max="2099"
                       style="width: 100%; border: none; border-bottom: 2px solid var(--border); padding: 8px 0; font-size: 14px; color: var(--text-1); outline: none; background: transparent; box-sizing: border-box;"
                       onfocus="this.style.borderColor='var(--accent)'" onblur="this.style.borderColor='var(--border)'">
                @error('year')<p style="font-size: 12px; color: var(--danger); margin-top: 4px;">{{ $message }}</p>@enderror
            </div>

            <div style="display: flex; gap: 12px;">
                <button type="submit"
                        style="background: var(--accent); color: white; border: none; padding: 11px 24px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; letter-spacing: 0.5px;">
                    Salvar turma
                </button>
                <a href="{{ route('secretaria.turmas.index') }}"
