@php
    ...
    $readonlyBlock = fn(string $title, string $subtitle, string $text) =>
        '<div style="background: var(--info-bg); border: 1px solid var(--info-border); border-radius: 10px; padding: 20px; margin-bottom: 24px;">
            <p style="font-size: 11px; font-weight: 700; color: var(--accent-text); letter-spacing: 1px; text-transform: uppercase; margin: 0 0 10px;">' . e($title) . '</p>
            <p style="font-size: 12px; color: var(--text-3); font-style: italic; margin: 0 0 10px;">' . e($subtitle) . '</p>
    ...
    $emptyReadonly = fn(string $title, string $subtitle) =>
        '<div style="background: var(--bg-subtle); border: 1px solid var(--border); border-radius: 10px; padding: 16px 20px; margin-bottom: 24px;">
            ...
            <p style="font-size: 12px; color: var(--text-4); font-style: italic; margin: 0;">' . e($subtitle) . '</p>
        </div>';
@endphp
...
<div style="margin-bottom: 28px;">
    <div style="border-left: 3px solid {{ $accent }}; padding: 4px 0 4px 14px; margin-bottom: 20px;">
        <p style="font-size: 15px; font-weight: 700; color: var(--text-1); margin: 0 0 2px;">Objetivos de Aprendizagem</p>
        <p style="font-size: 12px; color: var(--text-4); font-style: italic; margin: 0;">Metas estabelecidas para o período letivo.</p>
    </div>
    ...
    <div style="margin-bottom: 20px;">
        <label style="display: block; font-size: 12px; font-weight: 600; color: var(--text-2); margin-bottom: 4px;">
            <span style="display: inline-block; padding: 2px 10px; border-radius: 20px; font-size: 11px; font-weight: 700; background: var(--bg-subtle); color: var(--text-2); margin-right: 6px;">{{ $badge }}</span>
            {{ $label }}
        </label>
        <textarea name="{{ $field }}" rows="3"
                  ...
                  onfocus="this.style.borderColor='{{ $accent }}'" onblur="this.style.borderColor='var(--border)'">{{ $fn($field) }}</textarea>
    </div>
    @endforeach
</div>
...
<div style="margin-bottom: 28px;">
    <div style="border-left: 3px solid {{ $accent }}; padding: 4px 0 4px 14px; margin-bottom: 16px;">
        <p style="font-size: 15px; font-weight: 700; color: var(--text-1); margin: 0 0 2px;">Estratégias Pedagógicas</p>
        <p style="font-size: 12px; color: var(--text-4); font-style: italic; margin: 0;">Metodologias e adaptações a serem adotadas.</p>
    </div>
    <textarea name="estrategias_pedagogicas" rows="5"
              style="width: 100%; border: none; border-bottom: 2px solid var(--border); padding: 8px 0; font-size: 14px; color: var(--text-1); outline: none; resize: vertical; box-sizing: border-box; font-family: inherit; line-height: 1.7; background: transparent;"
              onfocus="this.style.borderColor='{{ $accent }}'" onblur="this.style.borderColor='var(--border)'">{{ $fn('estrategias_pedagogicas') }}</textarea>
</div>
...
<div style="margin-bottom: 28px;">
    ...
    <textarea name="criterios_avaliacao" rows="4"
              style="width: 100%; border: none; border-bottom: 2px solid var(--border); padding: 8px 0; font-size: 14px; color: var(--text-1); outline: none; resize: vertical; box-sizing: border-box; font-family: inherit; line-height: 1.7; background: transparent;"
              onfocus="this.style.borderColor='{{ $accent }}'" onblur="this.style.borderColor='var(--border)'">{{ $fn('criterios_avaliacao') }}</textarea>
</div>
...
@if($equipePreenchida)
<div style="background: var(--info-bg); border: 1px solid var(--info-border); border-radius: 10px; padding: 20px; margin-bottom: 24px;">
    <p style="font-size: 11px; font-weight: 700; color: var(--accent-text); letter-spacing: 1px; text-transform: uppercase; margin: 0 0 14px;">Equipe Responsável — extraído do Estudo de Caso (Termo de Ciência)</p>
