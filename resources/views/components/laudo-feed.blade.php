...
<div style="background: var(--bg-card); border-radius: 12px; border: 1px solid var(--border-sub); padding: 24px; margin-bottom: 16px;">
    <h3 style="font-size: 14px; font-weight: 600; color: var(--text-1); margin: 0 0 16px;">{{ term('laudos') }}</h3>

    @if($role === 'secretaria')
    <form method="POST"
          action="{{ route('secretaria.alunos.laudos.store', $aluno) }}"
          enctype="multipart/form-data"
          style="margin-bottom: 20px; padding-bottom: 20px; border-bottom: 1px solid var(--border-sub);">
        @csrf
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
            <div>
                ...
                @error('tipo')<p style="font-size: 12px; color: var(--danger); margin: 4px 0 0;">{{ $message }}</p>@enderror
            </div>
            <div>
                ...
                <input type="date" name="data_laudo" value="{{ old('data_laudo') }}"
                       style="width: 100%; border: none; border-bottom: 2px solid var(--border); padding: 8px 0; font-size: 14px; color: var(--text-1); outline: none; background: transparent; box-sizing: border-box;">
                @error('data_laudo')<p style="font-size: 12px; color: var(--danger); margin: 4px 0 0;">{{ $message }}</p>@enderror
            </div>
        </div>
        ...
        <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
            <label id="laudo-file-label-{{ $aluno->id }}"
                   style="display: inline-flex; align-items: center; gap: 8px; background: var(--bg-subtle); padding: 8px 14px; border-radius: 8px; cursor: pointer; font-size: 13px; color: var(--text-2); font-weight: 500;">
            ...
            <button type="submit"
                    style="background: var(--accent); color: white; border: none; padding: 9px 20px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer;">
                Enviar {{ strtolower(term('laudo')) }}
            </button>
        </div>
        @error('arquivo')<p style="font-size: 12px; color: var(--danger); margin-top: 8px;">{{ $message }}</p>@enderror
    </form>
    @endif
    ...
    @forelse($aluno->laudos as $laudo)
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 12px 0; {{ !$loop->last ? 'border-bottom: 1px solid #F9FAFB;' : '' }}">
            <div style="display: flex; align-items: center; gap: 12px;">
                <div style="width: 36px; height: 36px; border-radius: 8px; background: var(--accent-bg); display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#004B8D" stroke-width="2">
                        <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><path d="M14 2v6h6"/>
                    </svg>
                </div>
                <div>
                    <p style="font-size: 13px; font-weight: 600; color: var(--text-1); margin: 0;">{{ $laudo->tipo_label }}</p>
    ...
    @empty
        <p style="font-size: 13px; color: var(--text-4);">Nenhum {{ strtolower(term('laudo')) }} cadastrado.</p>
    @endforelse
</div>